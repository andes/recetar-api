...
passport.use('jwt', new JwtStrategy({
    ...
    try {
        ...
        const user = await User.findOne({ _id: payload.sub }).select('_id lastLogin isActive');
        ...
        if (isSessionExpired(user.lastLogin)) {
            ...
            return done(null, false, { code: httpCodes.UNAUTHORIZED, message: SESSION_EXPIRED_MESSAGE });
        }
...
const authenticationMiddleware = (req: Request, res: Response, next: NextFunction, authenticationType: string) => {
    passport.authenticate(authenticationType, { session: false }, (err, user: IUser | boolean, info?: { code: number; message: string }): any | Response => {
        try {
            ...
            if (typedError.code === 'ERR_HTTP_INVALID_STATUS_CODE') {
                return res.status(httpCodes.EXPECTATION_FAILED).json({ message: 'Debe iniciar sesión' });
            }
