...
class PrescriptionPublicController implements BaseController {
  ...
  public getByUserId = async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      const prescriptions: IPrescription[] | null = await Prescription.find({ "professional.userId": userId, "ambito": "publico" }).sort({ field: 'desc', date: -1 });
      return res.status(200).json(prescriptions);
    } catch (err) {
      console.log(err);
      return res.status(500).json('Server Error');
  ...
  public dispense = async (req: Request, res: Response) => {
    try {
      ...
      const prescription: IPrescription | null = await Prescription.findOneAndUpdate({ _id: id, status: 'Pendiente', 'ambito': 'publico' }, {
        status: 'Dispensada',
        dispensedBy: {
  ...
  public cancelDispense = async (req: Request, res: Response) => {
    try {
      ...
      const controlPrescription: IPrescription | null = await Prescription.findOne({ _id: id, status: 'Dispensada', 'ambito': 'publico' });
      ...
      const opts: any = { new: true };
      const prescription: IPrescription | null = await Prescription.findOneAndUpdate({ _id: id, status: 'Dispensada', 'ambito': 'publico' }, {
        status: 'Pendiente',
        dispensedBy: {},
        dispensedAt: ''
      }, opts);
  ...
  public update = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
      ...
      const prescription: IPrescription | null = await Prescription.findOne({ _id: id, status: "Pendiente", 'ambito': 'publico' });
      ...
      const updatedPrescription: IPrescription | null = await Prescription.findOneAndUpdate({ _id: id, 'ambito': 'publico'}, {
        date,
        observation,
        diagnostic,
...
export default new PrescriptionPublicController();
