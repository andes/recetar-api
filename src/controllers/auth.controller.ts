    ...
    try {
      ...
      if (roleType === "professional") {
        ...
        const matchesProfesional = profAut.some((p: any) => {
          ...
          const fechaEgresoMatches = moment(fechaEgreso).isSame(moment(p.fechaEgreso), 'day');
        ...
        if (!matchesProfesional) {
          return res.status(400).json({ message: 'No es posible registrar,datos de profesional no coinciden' });
        }
