...
class AuthController {
    ...
    public register = async (req: Request, res: Response): Promise<Response> => {
        try {
            ...
            if (posibleExistingUser) return res.status(400).json({
                message: `No es posible registrar, el usuario ${username} ya existe`
            });
            ...
            if (roleType === "professional") {
                ...
                const profesiones = professionalAndes.profesiones;
                ...
                const vigencia = moment(lastMatriculacion.fin).diff(moment(), 'days');
                const usu = { username, email, password, enrollment, cuil, businessName };
                if (lastMatriculacion && vigencia > 0 && lastMatriculacion.matriculaNumero.toString() === enrollment && cuil === professionalAndes.cuit) {
   ...
   */
    ...
    public getPharmacyAndes = async (req: Request, res: Response): Promise<Response> => {
        try {
            ...
            const resp = await needle('get', `${process.env.ANDES_ENDPOINT_DEV}/core/tm/farmacias?cuit=${cuil}&disposicionHabilitacion=${disposicionHabilitacion}`, { headers: { 'Authorization': process.env.JWT_LOCAL_TOKEN } });
