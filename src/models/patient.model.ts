...
Patient.schema.method('findOrCreate', async (patientParam: IPatient): Promise<IPatient> => {
    try {
        ...
        const idPatient = patient ? patient.id : null;
        if (!idPatient || patient?.idMPI === '') {
            const url = `${process.env.ANDES_MPI_ENDPOINT}?search=${patientParam.dni}&sexo=${sexo}&activo=true&estado=validado`;
            const resp = await needle('get', url, { headers: { Authorization: process.env.JWT_MPI_TOKEN } });
            resp.body.forEach(async (item: any) => {
                if (item.sexo === patientParam.sex.toLocaleLowerCase()) {
                    ...
                    if (!idPatient) {
                        patient = await new Patient(patient);
                        await patient.save();
                    } else {
                        await Patient.updateOne({ _id: idPatient }, patient);
